...
%%  ******************************************************  %%
%%                                                          %%
%% Chapter 4: Fig 4.14 & 4.15                               %%
%%                                                          %%
%% Author: Frank Gross                                      %%
%% McGraw-Hill, 2005                                        %%
%% Date:  4/24/2005                                         %%
%%                                                          %%
%% This code creates Figures 4.14 & 4.15, plots of Blackman %%
...
if check == 0    % If the array is even
    wB = flipud(test(1:N/2));  wB = wB/max(wB);
    figure(1), stem(test/max(test),'k')
    xlabel('Element Number'), ylabel('Array Weights')
    title('\bfFigure 4.14 - Blackman Array Weights vs. Element Number')
    axis([1 N 0 1.1])
    AF = 0;
    tot = sum(wB);
    for i = 1:N/2
...
title('\bfFigure 4.15 - Blackman Weighted Array Factor vs. Angle')
